...
This is the Stateless fork of DPDK. Instead of maintaining a normal git fork we
use [stgit](http://www.procode.org/stgit/) to maintain a patchset. This
...
```
git clone git@github.com:BeStateless/dpdk.git
cd dpdk
./patch
```

Now the patched code is in the `dpdk` directory in the repository root with
...
`dpdk` directory to export the new patch. Modifying an existing patch,
